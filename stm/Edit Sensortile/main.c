...
#if (defined(STM32_SENSORTILE) || defined(STM32_BLUECOIN))
static uint32_t SendBatteryInfoData(void);
#endif /* STM32_SENSORTILE */
...
uint16_t sampleServHandle, TXCharHandle, RXCharHandle, QuaternionsPCCharHandle, BatteryPCCharHandle;
tBleStatus Add_Sample_Service(void);
void sendDataTX(uint8_t* data_buffer, uint8_t Nb_bytes);
void sendDataTX32(uint32_t* data_buffer, uint32_t Nb_bytes);
tBleStatus QuatPC_Update(SensorAxes_t *data, uint32_t soc);
tBleStatus BattPC_Update(uint32_t *data);
...
int main(void)
{
  ...
  {
    /* Configure RTC Data */
    /* Set Default Date: Monday October 30th 2017 */
...
static void ComputeQuaternions(void)
{
  ...
  if(W2ST_CHECK_CONNECTION(W2ST_CONNECT_EC)) {
    ...
    if(CounterFX==QUAT_UPDATE_MUL_10MS){
      ...
      //olivier
      uint8_t data[20] = {'Q','U','A','T','E','R','6','7','8','9','A','B','C','D','E','F','G','H','I','J'};
      //sendDataTX use for debug
      sendDataTX(data, sizeof(data));
      QuatPC_Update(quat_axes, SendBatteryInfoData());
      //olivier


    }
/*
    ALLMEMS1_PRINTF("%u ;QX=%ld ; QY=%ld ; QZ=%ld ; QW=%ld \r\n",
    		HAL_GetTick(),
    		quat_axes[QuaternionNumber].AXIS_X,
			quat_axes[QuaternionNumber].AXIS_Y,
    		quat_axes[QuaternionNumber].AXIS_Z,
			(int32_t)(MotionFX_Engine_Out->quaternion_9X[3] * 10000));
*/
  }
}
/* Code for MotionFX integration - End Section */
...
static void Init_BlueNRG_Stack(void)
{
...
#ifdef MAC_BLUEMS
  {
    ...
    //uint8_t tmp_bdaddr[6]= {0x50, 0x56, 0x31, 0x3A, 0x85, 0xC0};
    int32_t i;
    for(i=0;i<6;i++)
...
#define olivier
  ...
  if(ret != BLE_STATUS_SUCCESS){
    ...
    uint8_t SERVER_BDADDR[] = {0x50, 0x56, 0x31, 0x3A, 0x85, 0xC0};
...
tBleStatus Add_Sample_Service(void)
{
  ...
  */
  ...
  const uint8_t charUuidQuater[16] = {0x66,0x9a,0x0c,0x20,0x00,0x08,0x96,0x9e,0xe2,0x11,0x9e,0xb1,0xe3,0xf2,0x73,0xd9};
  const uint8_t charUuidBatt[32] = {0x66,0x9a,0x0c,0x20,0x00,0x08,0x96,0x9e,0xe2,0x11,0x9e,0xb1,0xe3,0xf2,0x73,0xd9};

  uint8_t max_attr_records= 2; //tx et RX
  max_attr_records++; //quatter


  ret = aci_gatt_add_serv(UUID_TYPE_128, service_uuid, PRIMARY_SERVICE, 1+ 3*max_attr_records, &sampleServHandle); /* original is 9?? */
  ...
  ret =  aci_gatt_add_char(sampleServHandle, UUID_TYPE_128, charUuidQuater, 2+6*SEND_N_QUATERNIONS,
                             CHAR_PROP_NOTIFY,
                             ATTR_PERMISSION_NONE,
                             GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
                             16, 0, &QuaternionsPCCharHandle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;

  ret =  aci_gatt_add_char(sampleServHandle, UUID_TYPE_128, charUuidBatt, 32,
                             CHAR_PROP_NOTIFY,
                             ATTR_PERMISSION_NONE,
                             GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
                             32, 0, &BatteryPCCharHandle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;




  ALLMEMS1_PRINTF("Sample Service added.\nTX Char Handle %04X, RX Char Handle %04X, quatter Char Handle %04X\r\n", TXCharHandle, RXCharHandle,QuaternionsPCCharHandle);
  return BLE_STATUS_SUCCESS;

fail:
ALLMEMS1_PRINTF("Error while adding Sample Service.\r\n");
...
#error SEND_N_QUATERNIONS could be only 1,2,3
  ...
  ret = aci_gatt_update_char_value(sampleServHandle, BatteryPCCharHandle, 0, soc, 32);

  if (ret != BLE_STATUS_SUCCESS){
    if(W2ST_CHECK_CONNECTION(W2ST_CONNECT_STD_ERR)){
      BytesToWrite =sprintf((char *)BufferToWrite, "Error Updating Batt Charge\r\n");
      Stderr_Update(BufferToWrite,BytesToWrite);
    } else {
      ALLMEMS1_PRINTF("Error Updating Batt Charge\r\n");
    }
    return BLE_STATUS_ERROR;
  }
  return BLE_STATUS_SUCCESS;
...
//olivier
/*
#define STORE_LA_BATT(buf, val)    ( ((buf)[0] =  (uint8_t) (val)    ) , \
                                   ((buf)[1] =  (uint8_t) (val>>8) ) )
...
#if data != null
  STORE_LA_BATT(buff,data[0]);
#else
  printf("An error occurred charge data equals none!");
//#error Data can not be equals a none
#endif
  ret = aci_gatt_update_char_value(sampleServHandle, BatteryPCCharHandle, 0, 1, buff);
